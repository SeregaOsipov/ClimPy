...
# args.wrf_out = '/scratch/osipovs/Data/AirQuality/THOFA/inversion/v4/run_srs_ref/pp/column_loading/wrfout_d01_2023-06-10_00_00_00'
...
pressure, pressure_stag, p_sfc, dp, z_stag, dz = compute_stag_pressure(nc_in, time_index=time_index, squeeze=False)

# compute column average (not Dobson units)
for var_key in TROPOMI_in_WRF_KEYS:
    gas_ppmv = wrf.getvar(nc_in, var_key, timeidx=time_index, squeeze=False)
    gas_column_da = gas_ppmv.weighted(dp/dp.sum(dim='bottom_top')).sum(dim='bottom_top')  # column average gas ppmv
    gas_column_da.name = '{}'.format(var_key)
    gas_column_da.attrs['units'] = xr_in[var_key].units
    gas_column_da.attrs['description'] = 'column average {}, mimics TROPOMI diag'.format(var_key)

...
def derive_merra2_pressure_profile(merra_ds, hPa=False):
    ...
    layer_pressure_thickness = merra_ds['DELP'].sortby('lev')  # sort to make sure that summation takes places from top
    ...
    if not all(merra_ds['DELP'].lev.to_numpy() == layer_pressure_thickness.lev.to_numpy()):  # in MERRA2 first layer by index is TOA
    ...
    toa_pressure = pressure_stag_no_toa.sel(lev=1, drop=False).copy(deep=True)  # MERRA2 has first lev at TOA
    toa_pressure[:] = 1
    toa_pressure['lev'] = 0
